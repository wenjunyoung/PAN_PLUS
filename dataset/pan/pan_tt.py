...
def shrink(contours, bboxes, rate, max_shr=20):
    rate = rate * rate
    shrinked_bboxes = []
...
class PAN_TT(data.Dataset):
    ...
    def prepare_train_data(self, index):
        ...
        contours, bboxes, words = get_ann(img, gt_path)
        ...
        for rate in [self.kernel_scale]:
            ...
            kernel_bboxes = shrink(contours, bboxes, rate)
            for i in range(len(bboxes)):
